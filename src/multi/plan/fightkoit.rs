...
impl PartialOrd for FightKOITPlanState {
  fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
    if self.num_turns != other.num_turns {
      let enemy_hp = if let PlanState::FightTurnITState(FightTurnITPlanState { enemy_hp, .. }) = *self.inner_plan { enemy_hp } else { panic!("unexpected inner plan state") };
      let other_enemy_hp = if let PlanState::FightTurnITState(FightTurnITPlanState { enemy_hp, .. }) = *other.inner_plan { enemy_hp } else { panic!("unexpected inner plan state") };
      if self.num_turns < other.num_turns {
...
impl PartialEq for FightKOITPlanState {
  fn eq(&self, other: &Self) -> bool {
    self.partial_cmp(other) == Some(Ordering::Equal)
  }
}

pub struct FightKOITPlan<R> {
  num_turns: u16,
  move_order: MoveOrder,
  non_crit_damage: RangeInclusive<u16>,
  crit_damage: RangeInclusive<u16>,

  fight_turn_plan: Option<FightTurnITPlan<R>>,
...
impl<R: MultiRom + HandleMenuInputAddresses + BattleMovesInfoAddresses + BattleMonInfoAddresses + AIChooseMoveAddresses + BattleDetermineMoveOrderAddresses + BattleObedienceAddresses + Gen1TrainerAIAddresses + Gen1FightTurnAddresses + Gen1MoveEffectAddresses> FightKOITPlan<R> {
  fn create_fight_turn_plan(&mut self) -> FightTurnITPlan<R> {
    ...
    FightTurnITPlan::new(AttackDesc::hit_or_crit_effect(self.mov, self.non_crit_damage.clone(), self.crit_damage.clone(), effect), enemy_attack, move_order)
  }
...
impl<R: MultiRom + HandleMenuInputAddresses + BattleMovesInfoAddresses + BattleMonInfoAddresses + AIChooseMoveAddresses + BattleDetermineMoveOrderAddresses + BattleObedienceAddresses + Gen1TrainerAIAddresses + Gen1FightTurnAddresses + Gen1MoveEffectAddresses> Plan<R> for FightKOITPlan<R> {
  type Value = ();

  fn save(&self) -> PlanState {
    PlanState::FightKOITState(FightKOITPlanState { num_turns: self.num_turns, move_order: self.move_order, non_crit_damage: self.non_crit_damage.clone(), crit_damage: self.crit_damage.clone(), inner_plan: Rc::new(self.fight_turn_plan.as_ref().unwrap().save()) })
  }
  fn restore(&mut self, state: &PlanState) {
    if let PlanState::FightKOITState(FightKOITPlanState { num_turns, move_order, non_crit_damage, crit_damage, inner_plan }) = state {
