...
pub struct BuySellQuitMenuPlanState {
  handle_menu_input_state: HandleMenuInputState,
  goal_dist: u8
}
impl PartialOrd for BuySellQuitMenuPlanState {
  fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
    other.goal_dist.partial_cmp(&self.goal_dist)
  }
}
impl PartialEq for BuySellQuitMenuPlanState {
...
pub struct BuySellQuitMenuPlan {
  // instance state
  handle_menu_input_state: HandleMenuInputState,
...
impl BuySellQuitMenuPlan {
  pub fn buy() -> Self { Self::with_choose_option(0) }
  pub fn sell() -> Self { Self::with_choose_option(1) }
...
impl<R: Rom + JoypadLowSensitivityAddresses + HandleMenuInputAddresses + InputIdentificationAddresses> Plan<R> for BuySellQuitMenuPlan {
  type Value = ();

  fn save(&self) -> PlanState {
    PlanState::BuySellQuitMenuState(BuySellQuitMenuPlanState { handle_menu_input_state: self.handle_menu_input_state.clone(), goal_dist: self.goal_dist(self.handle_menu_input_state.current_item) })
  }
  fn restore(&mut self, state: &PlanState) {
    if let PlanState::BuySellQuitMenuState(BuySellQuitMenuPlanState { handle_menu_input_state, goal_dist: _ }) = state {
  ...
  fn canonicalize_input(&self, input: Input) -> Option<Input> {
    match self.handle_menu_input_state.get_result(input) {
      ...
      HandleMenuInputResult::Exit { current_item, input: exit_input } => {
        if exit_input.intersects(A) || !exit_input.intersects(B) {
