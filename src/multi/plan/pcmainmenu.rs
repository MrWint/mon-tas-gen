...
pub struct PCMainMenuPlanState {
  handle_menu_input_state: HandleMenuInputState,
  goal_dist: u8
}
impl PartialOrd for PCMainMenuPlanState {
  fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
    other.goal_dist.partial_cmp(&self.goal_dist)
  }
}
impl PartialEq for PCMainMenuPlanState {
...
impl PCMainMenuPlan {
  pub fn mon() -> Self { Self::with_choose_option(0) }
  pub fn items() -> Self { Self::with_choose_option(1) }
  pub fn dex() -> Self { Self::with_choose_option(2) }
  pub fn league() -> Self { Self::with_choose_option(3) }
  pub fn quit() -> Self { Self::with_choose_option(4) }
...
impl<R: Rom + JoypadLowSensitivityAddresses + HandleMenuInputAddresses + InputIdentificationAddresses> Plan<R> for PCMainMenuPlan {
  type Value = ();

  fn save(&self) -> PlanState {
    PlanState::PCMainMenuState(PCMainMenuPlanState { handle_menu_input_state: self.handle_menu_input_state.clone(), goal_dist: self.goal_dist(self.handle_menu_input_state.current_item) })
  }
  fn restore(&mut self, state: &PlanState) {
    if let PlanState::PCMainMenuState(PCMainMenuPlanState { handle_menu_input_state, goal_dist: _ }) = state {
