...
impl<R: MultiRom + JoypadOverworldAddresses + Gen1OverworldAddresses + Gen1DVAddresses> Plan<R> for OverworldOpenStartMenuPlan {
  ...
  fn save(&self) -> PlanState {
    PlanState::OverworldOpenStartMenuState { joypad_overworld_state: self.joypad_overworld_state.clone() }
  }
  fn restore(&mut self, state: &PlanState) {
    if let PlanState::OverworldOpenStartMenuState { joypad_overworld_state, } = state {
