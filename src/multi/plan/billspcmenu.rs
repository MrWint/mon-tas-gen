...
pub struct BillsPCMenuPlanState {
  handle_menu_input_state: HandleMenuInputState,
  goal_dist: u8
}
impl PartialOrd for BillsPCMenuPlanState {
  fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
    other.goal_dist.partial_cmp(&self.goal_dist)
  }
}
impl PartialEq for BillsPCMenuPlanState {
  fn eq(&self, other: &Self) -> bool {
    self.partial_cmp(other) == Some(Ordering::Equal)
  }
}

// Plan to progress HandleMenuInput_ inputs, selecting an option in the Bill's PC menu
pub struct BillsPCMenuPlan {
  // instance state
  handle_menu_input_state: HandleMenuInputState,
...
impl BillsPCMenuPlan {
  pub fn withdraw() -> Self { Self::with_choose_option(0) }
  pub fn deposit() -> Self { Self::with_choose_option(1) }
  pub fn release() -> Self { Self::with_choose_option(2) }
  pub fn cahnge_box() -> Self { Self::with_choose_option(3) }
  pub fn print_box() -> Self { Self::with_choose_option(4) }
  pub fn quit() -> Self { Self::with_choose_option(5) }
  ...
  fn goal_dist(&self, current_item: u8) -> u8 {
    if self.choose_option == 5 { 0 } else if current_item > self.choose_option { current_item - self.choose_option } else { self.choose_option - current_item }
  }
}
impl<R: Rom + JoypadLowSensitivityAddresses + HandleMenuInputAddresses + InputIdentificationAddresses> Plan<R> for BillsPCMenuPlan {
  type Value = ();

  fn save(&self) -> PlanState {
    PlanState::BillsPCMenuState(BillsPCMenuPlanState { handle_menu_input_state: self.handle_menu_input_state.clone(), goal_dist: self.goal_dist(self.handle_menu_input_state.current_item) })
  }
  fn restore(&mut self, state: &PlanState) {
    if let PlanState::BillsPCMenuState(BillsPCMenuPlanState { handle_menu_input_state, goal_dist: _ }) = state {
  ...
  fn get_blockable_inputs(&self) -> Input { if self.choose_option < 5 { A } else { B } }
    ...
    match self.handle_menu_input_state.get_result(input) {
      ...
      HandleMenuInputResult::Exit { current_item, input: exit_input } => {
        if !exit_input.intersects(B) {
          ...
          if self.choose_option == 5 { Some(B) } else { None }
        }
      },
    }
  }
    ...
    match self.handle_menu_input_state.get_result(input) {
      ...
      HandleMenuInputResult::Exit { current_item, input: exit_input } => {
        if !exit_input.intersects(B) {
          ...
          if self.choose_option != 5 { return None; }
        }
        gb.restore(s);
        gb.input(input);
        gb.step();
