...
pub struct PartyMonMenuPlanState {
  handle_menu_input_state: HandleMenuInputState,
  goal_dist: u8
}
impl PartialOrd for PartyMonMenuPlanState {
  fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
    other.goal_dist.partial_cmp(&self.goal_dist)
  }
}
impl PartialEq for PartyMonMenuPlanState {
  fn eq(&self, other: &Self) -> bool {
    self.partial_cmp(other) == Some(Ordering::Equal)
  }
}

// Plan to progress HandleMenuInput_ inputs, selecting an option in the mart buy/sell/quit menu
pub struct PartyMonMenuPlan {
  // instance state
  handle_menu_input_state: HandleMenuInputState,
...
impl PartyMonMenuPlan {
  pub fn quit() -> Self { Self::choose(0xff) }
  ...
  fn goal_dist(&self, current_item: u8) -> u8 {
    if self.choose_option == 0xff { 0 } else if current_item > self.choose_option { current_item - self.choose_option } else { self.choose_option - current_item }
  }
}
impl<R: Rom + JoypadLowSensitivityAddresses + HandleMenuInputAddresses + InputIdentificationAddresses> Plan<R> for PartyMonMenuPlan {
  type Value = ();

  fn save(&self) -> PlanState {
    PlanState::PartyMonMenuState(PartyMonMenuPlanState { handle_menu_input_state: self.handle_menu_input_state.clone(), goal_dist: self.goal_dist(self.handle_menu_input_state.current_item) })
  }
  fn restore(&mut self, state: &PlanState) {
    if let PlanState::PartyMonMenuState(PartyMonMenuPlanState { handle_menu_input_state, goal_dist: _ }) = state {
  ...
  fn get_blockable_inputs(&self) -> Input { if self.choose_option < 0xff { A } else { B } }
    ...
    match self.handle_menu_input_state.get_result(input) {
      ...
      HandleMenuInputResult::Exit { current_item, input: exit_input } => {
        if !exit_input.intersects(B) {
          ...
          if self.choose_option == 0xff { Some(B) } else { None }
        }
      },
    }
  }
    ...
    match self.handle_menu_input_state.get_result(input) {
      ...
      HandleMenuInputResult::Exit { current_item, input: exit_input } => {
        if !exit_input.intersects(B) {
          ...
          if self.choose_option != 0xff { return None; }
        }
        gb.restore(s);
        gb.input(input);
        gb.step();
