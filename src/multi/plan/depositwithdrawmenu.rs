...
pub struct DepositWithdrawMenuPlanState {
  handle_menu_input_state: HandleMenuInputState,
  goal_dist: u8
}
impl PartialOrd for DepositWithdrawMenuPlanState {
  fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
    other.goal_dist.partial_cmp(&self.goal_dist)
  }
}
impl PartialEq for DepositWithdrawMenuPlanState {
  fn eq(&self, other: &Self) -> bool {
    self.partial_cmp(other) == Some(Ordering::Equal)
  }
}

// Plan to progress HandleMenuInput_ inputs, selecting an option in the DisplayDepositWithdrawMenu
pub struct DepositWithdrawMenuPlan {
  // instance state
  handle_menu_input_state: HandleMenuInputState,
...
impl DepositWithdrawMenuPlan {
  pub fn withdraw() -> Self { Self::with_choose_option(0) }
  pub fn deposit() -> Self { Self::with_choose_option(0) }
  pub fn stats() -> Self { Self::with_choose_option(1) }
...
impl<R: Rom + JoypadLowSensitivityAddresses + HandleMenuInputAddresses + InputIdentificationAddresses> Plan<R> for DepositWithdrawMenuPlan {
  type Value = ();

  fn save(&self) -> PlanState {
    PlanState::DepositWithdrawMenuState(DepositWithdrawMenuPlanState { handle_menu_input_state: self.handle_menu_input_state.clone(), goal_dist: self.goal_dist(self.handle_menu_input_state.current_item) })
  }
  fn restore(&mut self, state: &PlanState) {
    if let PlanState::DepositWithdrawMenuState(DepositWithdrawMenuPlanState { handle_menu_input_state, goal_dist: _ }) = state {
