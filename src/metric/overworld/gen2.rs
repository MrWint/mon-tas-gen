...
pub fn get_overworld_interaction_result<R: JoypadAddresses + Gen2MapEventsAddresses>(gb: &mut dyn GbI<R>) -> OverworldInteractionResult {
...
impl<R: Rom + RoamMonAddresses> Metric<R> for RoamMonLocationMetric {
  ...
  fn evaluate(&self, gb: &mut dyn GbI<R>) -> Option<Self::ValueType> {
    if gb.step_until(&[R::AFTER_ROAM_MON_UPDATE_ADDRESS]) == 0 { return None; }
    for i in 0..3 {
