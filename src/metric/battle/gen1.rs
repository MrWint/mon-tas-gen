...
impl<R: JoypadAddresses + Gen1TrainerAIAddresses> Metric<R> for TrainerAIMetric {
  ...
  fn evaluate(&self, gb: &mut dyn GbI<R>) -> Option<Self::ValueType> {
    if gb.step_until(&[R::TRAINER_AI_START_ADDRESS]) == 0 { return None; }
...
impl<R: JoypadAddresses + Gen1MoveEffectAddresses> Metric<R> for MoveEffectMetric {
  ...
  fn evaluate(&self, gb: &mut dyn GbI<R>) -> Option<Self::ValueType> {
    if gb.step_until(&[R::MOVE_EFFECT_START_ADDRESS]) == 0 { log::debug!("MOVE_EFFECT_START_ADDRESS not found"); return None; }
    let hit = gb.step_until(&[
...
impl<R: JoypadAddresses + Gen1FightTurnAddresses> Metric<R> for Gen1NormalHitMetric {
  ...
  fn evaluate(&self, gb: &mut dyn GbI<R>) -> Option<Self::ValueType> {
    if gb.step_until(&[R::AFTER_MAX_DAMAGE_CALC_ADDRESS]) == 0 { log::debug!("AFTER_MAX_DAMAGE_CALC_ADDRESS not found"); return None; }
    let max_damage = gb.gb().read_memory_word_be(R::CUR_DAMAGE_MEM_ADDRESS);

    if gb.step_until(&[R::AFTER_PLAYER_HIT_CHECK_ADDRESS, R::AFTER_ENEMY_HIT_CHECK_ADDRESS]) == 0 { log::debug!("HIT_CHECK_ADDRESS not found"); return None; }
...
impl<R: JoypadAddresses + Gen1FightTurnAddresses> Metric<R> for Gen1MoveSuccessMetric {
  ...
  fn evaluate(&self, gb: &mut dyn GbI<R>) -> Option<Self::ValueType> {
    if gb.step_until(&[R::AFTER_PLAYER_HIT_CHECK_ADDRESS, R::AFTER_ENEMY_HIT_CHECK_ADDRESS]) == 0 { return None; }
    if gb.gb().read_memory(R::ATTACK_MISSED_MEM_ADDRESS) != 0 { return Some(FightTurnResult::Failed); }
