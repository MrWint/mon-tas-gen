...
pub struct WalkStepSegment<R: JoypadAddresses + RngAddresses + WalkStepAddresses> {
  skip_input: Input,
  debug_output: bool,
  _rom: PhantomData<R>,
}
impl<R: JoypadAddresses + RngAddresses + WalkStepAddresses> super::WithDebugOutput for WalkStepSegment<R> {
  fn with_debug_output(mut self) -> Self { self.debug_output = true; self }
}
impl<R: JoypadAddresses + RngAddresses + WalkStepAddresses> WalkStepSegment<R> {
  ...
  fn progress_print_letter_delay_frame(&self, gb: &mut Gb<R>, s: State) -> Vec<(State, u32)> {
    ...
    if delay >= 1 { // there's no way this input has an affect beyond PrintLetterDelay, there will always be a VBlank before leaving.
      ...
      if stayed { result.push((gb.save(), num_done + num_cycles)); } else {
        if !self.expect_conflicting_inputs { println!("WARNING: WalkStepSegment found state with conflicting inputs between PrintLetterDelay and future inputs [{}].", gb.get_stack_trace_string()); }
      }
    }
    result
  }
...
impl<R: JoypadAddresses + RngAddresses + WalkStepAddresses> super::Segment for WalkStepSegment<R> {
  type Rom = R;
  ...
  fn execute<I: IntoIterator<Item=State>>(&self, gb: &mut Gb<R>, iter: I) -> StateBuffer {
    ...
    for s in iter.into_iter() {
      ...
      if !super::is_correct_input_use(gb, Input::empty(), R::TEXT_BEFORE_JOYPAD_ADDRESS, R::TEXT_JOYPAD_ADDRESS, R::TEXT_AFTER_JOYPAD_ADDRESS) {
    ...
    while !active_states.is_empty() {
      ...
      if self.debug_output { println!("WalkStepSegment loop cycles {}-{}, min cycle size {}, goal_buffer size {}", min_cycles, max_cycles, sb.len(), goal_buffer.len()); }
