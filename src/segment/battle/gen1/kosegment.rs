...
impl<R: Rom + Gen1FightTurnAddresses + Gen1MoveEffectAddresses + BattleMovesInfoAddresses + BattleMonInfoAddresses + TextAddresses + BattleDetermineMoveOrderAddresses + BattleObedienceAddresses + AIChooseMoveAddresses + JoypadLowSensitivityAddresses + Gen1TrainerAIAddresses> Segment<R> for KOSegment {
  type Key = ();
  ...
  fn execute_split(&self, gbe: &mut RuntimeGbExecutor<R>, sb: StateBuffer) -> HashMap<Self::Key, StateBuffer> {
    ...
    while !active_states.is_empty() {
      ...
      if num_non_crits > 0 {
        ...
        for (damage, sb) in FightTurnSegment::new(self.mov, false, min_acceptable_damage, self.enemy_attack).with_expected_effect(if num_turns == 1 { None } else { self.expected_effect }).execute_split(gbe, sb.clone()) {
      ...
      if num_crits > 0 {
        ...
        for (damage, sb) in FightTurnSegment::new(self.mov, true, min_acceptable_damage, self.enemy_attack).with_expected_effect(if num_turns == 1 { None } else { self.expected_effect }).execute_split(gbe, sb.clone()) {
          let remaining_enemy_hp = enemy_hp - damage;
          if remaining_enemy_hp == 0 {
            goal_buffer.add_all(sb);
          } else {
