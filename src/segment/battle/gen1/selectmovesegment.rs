...
impl<R: Rom + Gen1FightTurnAddresses + BattleMovesInfoAddresses + BattleMonInfoAddresses + JoypadLowSensitivityAddresses> Segment<R> for SelectMoveSegment {
  type Key = ();

  fn execute_split(&self, gbe: &mut RuntimeGbExecutor<R>, sb: StateBuffer) -> HashMap<Self::Key, StateBuffer> {
    let sb = JoypadLowSensitivitySegment::new(&[Input::A]).with_buffer_size(self.buffer_size).execute(gbe, sb); // Fight
