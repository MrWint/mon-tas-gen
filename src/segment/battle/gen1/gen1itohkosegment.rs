...
pub struct Gen1ITOHKOSegment {
  mov: Move,
  buffer_size: usize,
}
impl Gen1ITOHKOSegment {
  #[allow(dead_code)]
...
impl WithOutputBufferSize for Gen1ITOHKOSegment {
  fn with_buffer_size(self, buffer_size: usize) -> Self { Self { buffer_size, ..self } }
}

impl<R: Rom + Gen1FightTurnAddresses + BattleMovesInfoAddresses + BattleMonInfoAddresses + BattleDetermineMoveOrderAddresses + BattleObedienceAddresses + JoypadLowSensitivityAddresses> Segment<R> for Gen1ITOHKOSegment {
  type Key = ();
  ...
  fn execute_split(&self, gbe: &mut RuntimeGbExecutor<R>, sb: StateBuffer) -> HashMap<Self::Key, StateBuffer> {
    ...
    if expect_crit {
      sb = Gen1ITSkipTextsSegment::new(1).with_buffer_size(self.buffer_size).execute(gbe, sb); // critical hit
    }
    if move_info.is_effective {
      sb = Gen1ITSkipTextsSegment::new(1).with_buffer_size(self.buffer_size).execute(gbe, sb); // (not) very effective
    }
