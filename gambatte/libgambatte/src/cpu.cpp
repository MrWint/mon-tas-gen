...
long CPU::runFor(unsigned long const cycles, bool startsOnFrameBoundaries) {
	mem_.setBasetime(cycleCounter_);
	process(cycles, startsOnFrameBoundaries);
...
void CPU::process(unsigned long const cycles, bool startsOnFrameBoundaries) {
	mem_.setEndtime(cycleCounter_, cycles);
	if (startsOnFrameBoundaries) mem_.updateInput();
	...
	while (mem_.isActive()) {
		...
		if (mem_.halted()) {
			...
			if (pc < 0x8000) {
				...
				uint32_t bank = 0;
				if (pc >= 0x4000) bank = mem_.curRomBank(), romAddress += (bank - 1) << 14;
				if (mem_.isInterrupt(romAddress)) {
...
void CPU::setInterruptAddresses(int *addrs, int numAddrs) {
	for (int i = 0; i < numInterruptAddresses; i++) mem_.clearInterrupt(fromInterruptAddress(interruptAddresses[i]));
	interruptAddresses = addrs;
	numInterruptAddresses = numAddrs;
	for (int i = 0; i < numInterruptAddresses; i++) mem_.setInterrupt(fromInterruptAddress(interruptAddresses[i]));
}
