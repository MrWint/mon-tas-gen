...
void Rtc::doLatch() {
	std::uint32_t tmp = ((dataDh & 0x40) ? haltTime : timeCB->getCurrentTime()) - baseTime;
...
void Rtc::setDh(const unsigned new_dh) {
	const std::uint32_t unixtime = (dataDh & 0x40) ? haltTime : timeCB->getCurrentTime();
	...
	if ((dataDh ^ new_dh) & 0x40) {
		if (new_dh & 0x40)
			haltTime = timeCB->getCurrentTime();
		else
			baseTime += timeCB->getCurrentTime() - haltTime;
	}
}
